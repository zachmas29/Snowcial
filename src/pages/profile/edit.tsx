...
import { Box, Button, Paper, TextField, Typography, FormControl, InputLabel, Select, MenuItem, OutlinedInput } from "@mui/material";
import { useRouter } from "next/router";
import { useEffect, useState } from "react";
import { useAuthContext } from "@/hooks/useAuth";
import { updateCurrentUserProfile } from "@/lib/db_functions";
import { uploadGalleryPhoto } from "@/lib/db_functions";
import { supabase } from "@/lib/supabase_client";
import type { Tables } from "@/types/database.types";
import ImageList from "@mui/material/ImageList";
import ImageListItem from "@mui/material/ImageListItem";
import { uploadProfilePhoto } from "@/lib/db_functions";
import { uploadBannerPhoto } from "@/lib/db_functions";
...
export default function EditProfilePage() {
  ...
  const [galleryPhotos, setGalleryPhotos] = useState<Tables<"gallery_photos">[]>([]);
  ...
  async function saveProfile() {
    ...
    info.nick_name = profile.nick_name || "";
    ...
    >
      ...
      <Box sx={{ px: 4, py: 3 }}>
        ...
        <ImageList
          ...
          rowHeight={240}
          sx={{
            "& .MuiImageListItem-root": { borderRadius: 2, overflow: "hidden" },
          }}
        >
          {[...Array(6)].map((_, i) => {
              ...
              return (
                <ImageListItem key={`placeholder-${i}`}>
                  ...
                  >
                    ...
                    >
                      ✎
                      <input type="file" hidden accept="image/*" onChange={handleGalleryChange} />
                    </Button>
                  </Box>
                </ImageListItem>
              );
            ...
            const { data: urlData } = supabase.storage.from("gallery-photos").getPublicUrl(photo.photo_path);
      ...
      <Box sx={{ px: 4, pb: 2 }}>
        <FormControl fullWidth>
          ...
          <Select
            ...
            renderValue={(selected) => selected.map((id) => tags.find((t) => t.id === id)?.name ?? "").join(", ")}
          >
            {tags.map((t) => (
      ...
      {/* Main Form */}
      <Box sx={{ p: 4 }}>
        <Box component="form" onSubmit={(e) => { e.preventDefault(); saveProfile(); }} sx={{ display: "flex", flexDirection: "column", gap: 2, width: "100%", maxWidth: 500, mx: "auto" }}>
          <TextField label="First Name" variant="outlined" value={profile.first_name} onChange={(e) => setProfile({ ...profile, first_name: e.target.value })} fullWidth />
          <TextField label="Last Name" variant="outlined" value={profile.last_name} onChange={(e) => setProfile({ ...profile, last_name: e.target.value })} fullWidth />
          <TextField label="Nickname" variant="outlined" value={profile.nick_name} onChange={(e) => setProfile({ ...profile, nick_name: e.target.value })} fullWidth />
          <TextField label="Bio" variant="outlined" multiline minRows={3} value={profile.bio_text} onChange={(e) => setProfile({ ...profile, bio_text: e.target.value })} fullWidth />
          <Button type="submit" variant="contained" sx={{ mt: 1, alignSelf: "center", textTransform: "none", borderRadius: 2, px: 4, py: 1, fontSize: "1rem", fontWeight: 600, backgroundColor: "black", color: "white", "&:hover": { backgroundColor: "#333" } }}>Save</Button>
        </Box>
      </Box>
    </Paper>
  </Box>
